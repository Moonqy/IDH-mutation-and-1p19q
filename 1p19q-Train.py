...
from models.qnetm import qnetm,qnetm0,qnetm1,qnetm2,qnetm3
...
parser.add_argument("--model_name", default="qnetm", type=str)
...
parser.add_argument("--tf", default=2, type=int)
parser.add_argument("--file", default=1, type=int)
...
if args.file==0:
    ...
    df_train = pd.read_csv(('../../brain-images/UCSF-1p19q2-train.csv'),usecols=['ID', 'IDH','age','gender','1p/19q','fold'])
images = []
...
def readIMGs(imgPath, csvFile, imgtype, N):
    ...
    for id, idh, age, q, fold in zip(csvFile['ID'], csvFile['IDH'], csvFile['age'], csvFile['1p/19q'], csvFile['fold']):
        if int(fold) in tfolds:
            ...
            if q ==0.0 or q==1.0 or q == 2.0:
                ...
                idh_onehot = torch.nn.functional.one_hot(torch.tensor(idh), num_classes=2).float()
                ...
                train_files.append({
                    "img1": img1_path,
                    "img2": img2_path,
                    "label": torch.tensor(q, dtype=torch.long),
...
def transformers():
    if args.tf == 0:
        ...
        [
            ...
            RandRotate90d(keys=["img1", "img2"], prob=0.5, spatial_axes=[0, 1]),
            CropForegroundd(keys=["img1","img2"],source_key="img1",allow_smaller=True),
            RandAffined(keys=["img1", "img2"],mode=("bilinear", "nearest"),prob=1.0,shear_range=(0.5,0.5,0.5), padding_mode="zeros",),
            Resized(keys=["img1","img2"], spatial_size=(130,170,130)),
        ])
    elif args.tf==3:
        train_transforms = Compose(
...
if args.file==1:
    ...
    def calculate_balanced_weights(num_samples_class0, num_samples_class1, num_samples_class2, smoothing_factor=0.1):
        total_samples = num_samples_class0 + num_samples_class1 + num_samples_class2
        
        # 使用 sqrt 来减少极端类别不平衡的影响
        weight_0 = torch.sqrt(torch.tensor(total_samples / (num_samples_class0 + smoothing_factor)))
        weight_1 = torch.sqrt(torch.tensor(total_samples / (num_samples_class1 + smoothing_factor)))
        weight_2 = torch.sqrt(torch.tensor(total_samples / (num_samples_class2 + smoothing_factor)))
        
        # 组合权重
        weights = torch.tensor([weight_0, weight_1, weight_2])
        
        # 归一化权重到合理范围
        min_weight = weights.min()
        max_weight = weights.max()
        weights = (weights - min_weight) / (max_weight - min_weight + 1e-6)
        weights = weights * 0.9 + 0.1  # 确保最小权重为 0.1
        return weights
    def get_class_distribution(files):
        class_counts = {0: 0, 1: 0, 2: 0}
        for file in files:
            q = file['label']
            class_counts[int(q)] += 1
        return class_counts
...
if __name__ == "__main__":
    ...
    elif args.model_name == 'qnetm':
        model = qnetm(out_channels=args.out_channels).to(device)
    elif args.model_name == 'qnetm0':
        model = qnetm(out_channels=args.out_channels).to(device)
    elif args.model_name == 'qnetm1':
        model = qnetm(out_channels=args.out_channels).to(device)
    elif args.model_name == 'qnetm2':
        model = qnetm(out_channels=args.out_channels).to(device)
    elif args.model_name == 'qnetm3':
        model = qnetm(out_channels=args.out_channels).to(device)
    ...
    if args.file==1:
        ...
        class_weights = calculate_balanced_weights(num_class_0, num_class_1).to(device)
    elif args.file==0:
        class_counts = get_class_distribution(trainlist)
        num_class_0 = class_counts[0]
        num_class_1 = class_counts[1]
        num_class_2 = class_counts[2]
        total_samples = len(train_labels)

        # 替换原来的 class_weights 计算
        # 计算类别权重
        class_weights = calculate_balanced_weights(num_class_0, num_class_1,num_class_2,smoothing_factor=0.1).to(device)
    '''
    ...
    for epoch in range(max_epochs):
        ...
        for step, batch_data in enumerate(epoch_iterator_train):
            step = step + 1
            inputs1,inputs2,age,idh,location, size, label = batch_data['img1'].to(device),batch_data['img2'].to(device), batch_data['age'].to(device),batch_data['idh'].to(device),batch_data['location'].to(device),batch_data['size'].to(device),batch_data['label'].to(device)
            optimizer.zero_grad()
            outputs = model(inputs1,inputs2,idh,location, size,age)
        ...
        with torch.no_grad():
            ...
            for val_data in test_loader:
                valimg1,valimg2,age,idh,location, size, val_labels = val_data["img1"].to(device), val_data["img2"].to(device), val_data['age'].to(device),val_data['idh'].to(device),val_data['location'].to(device),val_data['size'].to(device),val_data["label"].to(device)
                outputs=model(valimg1, valimg2,idh,location, size,age)
            ...
            if args.out_channels == 2:
                ...
                f1 = f1_score(y.cpu(), y_pred.cpu().argmax(dim=1), average='weighted')
                recall = recall_score(y.cpu(), y_pred.cpu().argmax(dim=1), average='weighted')
    ...
    if args.fold == 4:
        pd.DataFrame([result_data]).to_csv(os.path.join('/home/qy/pycode/MGMT/csv/1p19q.csv'), mode='a', header=False,
                                           index=False)  # mode??a,????csv???????
        calculate_average_metrics('/home/qy/pycode/MGMT/csv/1p19q.csv')
        pd.DataFrame([[]]).to_csv(os.path.join('/home/qy/pycode/MGMT/csv/1p19q.csv'), mode='a', header=False, index=False)
    else:
        pd.DataFrame([result_data]).to_csv(os.path.join('/home/qy/pycode/MGMT/csv/1p19q.csv'), mode='a', header=False,
                                           index=False)  # mode??a,????csv???????
